...
class PrintfulClient:
    ...
    api_record, created = mm.PODVendor.objects.get_or_create(
        code = 'PF',
        defaults = {
            'consumer_url': 'http://theprintful.com/',
            'dashboard_url': 'https://theprintful.com/dashboard/',
            'apibase_url': 'https://api.theprintful.com/'
        }
    )

    API_KEY = api_record.api_key.encode('utf-8')
    API_URL = api_record.apibase_url
    ...
    def __init__(self):
        try:
            tmp = mm.PODVendor.objects.get(code='PF')
            self.key = tmp.api_key.encode('UTF-8')
        except:
