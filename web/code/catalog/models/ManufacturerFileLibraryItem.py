...
class ManufacturerFileLibraryItem(models.Model):
    ...
    size = models.IntegerField(_("Size"), default=0, blank=True, null=True)
    width = models.IntegerField(_("Width"), default=0, blank=True, null=True)
    height = models.IntegerField(_("Height"), default=0)
    dpi = models.IntegerField(_("DPI"), default=0, blank=True, null=True)
    ...
    def get_resolution_string(self, dpi=300):
        return '{}" x {}"'.format(int(self.width / dpi), int(self.height / dpi))
    get_resolution_string.short_description = _("Resolution")
